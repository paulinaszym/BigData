...
val twittsDF = spark.readStream.format("kafka").option("kafka.bootstrap.servers", "server1.databricks.training:9092")
.option("subscribe", "tweets").option("maxOffsetsPerTrigger", 200)               
 .option("startingOffsets", "earliest").load().select($"value".cast("STRING"))                     
...
val analizaDF = twittsDF
 .withColumn("json", from_json(col("value"), twitSchema)).withColumn("json", from_json(col("value"), twitSchema))  
 .select(
   expr("cast(cast(json.createdAt as double)/1000 as timestamp) as time"), 
   $"json.hashTags".as("hashTags"),col("json.text").as("text"),
...
.select(explode(col("hashTags")).as("hashTag"), lower(col("hashTag")))      .groupBy(col("hashTag"))                   
 .count()                
 .orderBy(col("count"))                  
 .limit(30)                   
