...
for (x<-Seq.range(1,41)) dbutils.fs.cp(s"databricks-datasets/structured-streaming/events/file-$x.json", s"dbfs:/FileStore/streamDir/file-$x.json")
...
// MAGIC %sql select action, sum(count) as total_count from staticCountsDF group by action
...
// MAGIC %sql select action, date_format(window.end, "MMM-dd HH:mm") as time, count from staticCountsDF order by time, action
...
// MAGIC for (x<-Seq.range(41,50)) dbutils.fs.cp(s"databricks-datasets/structured-streaming/events/file-$x.json", s"/databricks-datasets/structured-streaming/events/file-$x.json")
// MAGIC 
// MAGIC // COMMAND ----------
